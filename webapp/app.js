...
function KassenbonManual() {
  ...
  function BudgetTab() {
    ...
    return /*#__PURE__*/React.createElement("div", { className: "px-4 mt-3 space-y-3" },
      ...
      /*#__PURE__*/React.createElement("div", { className: "bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-600 dark:border-slate-600 p-4 space-y-3" },
        ...
        /*#__PURE__*/React.createElement("button", { onClick: () => { const b = Number(budgetForm.einnahmeBetrag); if (!budgetForm.einnahmeQuelle.trim() || !(b > 0)) return; persistBudgetEinnahmen(budgetEinnahmen.concat([{ id: 'be' + Date.now(), quelle: budgetForm.einnahmeQuelle.trim(), betrag: b }])); setBudgetForm(Object.assign({}, budgetForm, { einnahmeQuelle: '', einnahmeBetrag: '' })); }, className: "w-full text-sm font-medium bg-emerald-600 text-white rounded-lg py-2" }, "Einnahme hinzufuegen")
      ...
      /*#__PURE__*/React.createElement("div", { className: "bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-600 dark:border-slate-600 p-4 space-y-3" },
        ...
        /*#__PURE__*/React.createElement("button", { onClick: () => { const b = Number(budgetForm.ausgabeBetrag); if (!budgetForm.ausgabeZweck.trim() || !(b > 0)) return; persistBudgetAusgaben(budgetAusgaben.concat([{ id: 'ba' + Date.now(), zweck: budgetForm.ausgabeZweck.trim(), betrag: b }])); setBudgetForm(Object.assign({}, budgetForm, { ausgabeZweck: '', ausgabeBetrag: '' })); }, className: "w-full text-sm font-medium bg-rose-600 text-white rounded-lg py-2" }, "Ausgabe hinzufuegen")
  ...
  window.collectAllData = function(){
    ...
    var data = { _meta: { app: 'ForderungenApp', version: APP_VERSION, exported: new Date().toISOString() } };
  ...
  window.writeSyncFile = function(data){
    ...
    try {
      ...
      console.log('[SYNC] export start, filename=' + filename);

      var trySave = function(attempt){

        try {

          if (typeof AndroidBridge !== 'undefined' && AndroidBridge && AndroidBridge.saveFile) {

            var payload = btoa(unescape(encodeURIComponent(json)));

            var ok = AndroidBridge.saveFile(filename, payload);

            console.log('[SYNC] saveFile ok=' + ok + ' attempt=' + attempt);

            if (ok) { console.log('[SYNC] ' + filename + ' exportiert nach Download'); localStorage.setItem('last-sync-export', JSON.stringify({ok:true, filename:filename, time:new Date().toISOString()})); return true; }

          }

        } catch(e){ console.log('[SYNC] saveFile attempt error=' + e.message); }

        if (attempt < 20) {

          setTimeout(function(){ trySave(attempt + 1); }, 200);
        ...
        try {
          ...
          alert(filename + ' heruntergeladen.');

          return true;

        } catch(e2){ alert('Download fehlgeschlagen: ' + e2.message); }

        alert('Export nicht moeglich: Bridge fehlt.');

        localStorage.setItem('last-sync-export', JSON.stringify({ok:false, filename:filename, time:new Date().toISOString()}));
    ...
    } catch (e) { alert('Sync-Export fehlgeschlagen: ' + e.message); }
  ...
  window.applySyncContent = function(content){
    ...
    try {
      ...
      // Auto-Login: ersten Admin in sessionStorage setzen (wie Android-App),
      // damit nach dem Reload direkt das Dashboard erscheint, nicht der Login/Setup.
      try {
        var uKey = 'forderungen-users-v1';
        var users = JSON.parse(localStorage.getItem(uKey) || '[]');
        var admin = users.filter(function(u){ return u && u.isAdmin; })[0] || users[0];
        if (admin) { sessionStorage.setItem('forderungen-session', JSON.stringify(admin)); }
      } catch (se) { console.log('[SYNC] session set skipped:', se.message); }
  ...
  function SyncBar(){
    ...
    b1.onclick = function(){ writeSyncFile(collectAllData()); };
...
var TAB_ORDER = ['dashboard','uebersicht','liste','sparen','nebenkosten','kassenbon','vergleich','sepa','budget'];
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: `${darkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-900'} px-5 pt-7 pb-6`
    ...
    className: "mono text-xs tracking-widest text-slate-200 uppercase"
  ...
  openChartItems.length > 0 && /*#__PURE__*/React.createElement("div", {
    ...
    onClick: () => writeSyncFile(collectAllData()),
    ...
    className: "relative px-5 mt-6 mb-8 text-center bg-slate-900 border-t border-slate-700 text-slate-100"
...
function Root() {
  ...
  useEffect(() => {
    ...
    if (isAndroid) {
      ...
      setTimeout(() => { try { writeSyncFile(collectAllData()); } catch (e) { console.log('[SYNC] startup error=' + e.message); } }, 10000);

      const t1 = setInterval(() => {

        try { writeSyncFile(collectAllData()); } catch (e) { console.log('[SYNC] interval error=' + e.message); }
  ...
  if (userMgmt && currentUser.isAdmin) {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: `${darkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-900'} px-5 pt-7 pb-5`
...
(function(){try{console.log("[SYNC] early export");var r=writeSyncFile(collectAllData());console.log("[SYNC] early export result="+JSON.stringify(r));}catch(e){console.log("[SYNC] early export error="+e.message);}})();
